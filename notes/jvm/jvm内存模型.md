...
5. **方法区**
   ...
   在老版的jdk中，方法区也被称为永久代。jdk8真正开始废弃永久代，而使用元空间（Metaspace）
...
1. **对象头【markword】**
   ...
   在32位系统下，对象头占8个字节，64位系统下则是16个字节【未开启压缩指针，开启后12个字节】。

   对象头在不同状态下存放的数据如下：

   ![](../../images/jvm/jvm-02.jpg)

   不同状态时对象头的区间含义如下图：

   ![](../../images/jvm/jvm-03.jpg)
...
2. **实例数据**
   ...
   分配策略：相同宽度的字段总是放在一起的，比如：double和long
...
![](../../images/jvm/jvm-04.jpg)

2. **直接指针**

   与句柄访问不同的是，ref中直接存储的就是对象的实例数据,但是类型数据跟句柄访问方式一样。

   优点:优势很明显，就是速度快，**相比于句柄访问少了一次指针定位的开销时间**。【可能是出于Java中对象的访问十分频繁,平时我们常用的JVM HotSpot采用此种方式】

![](../../images/jvm/jvm-05.jpg)
